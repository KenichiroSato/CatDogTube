...
def print_Text_Swift(strs):
    """
    generate Text.swift
    """
    out_dir = join(dirname(__file__), TEXT_SWIFT_DIR)
    if not isdir(out_dir):
        mkdir(out_dir)
    with open(join((out_dir), TEXT_SWIFT_NAME), "w") as f:
        f.write('//This class is generated by convert_string.py\n')
        f.write('//DO NOT modify this class manually\n\n')
        f.write('import Foundation\n\n')
        f.write('class Text {\n\n')
        for k in sorted(strs[ALT_LANG].keys(), key=lambda k: k.lower().replace("_", "0")):
            f.write('    static let ' + k + ' = NSLocalizedString("' + k +'", comment:"")\n\n')
        f.write('}')
